...
# --- ၃။ ဘာသာစကားနှင့် စာသားများ (Dictionary ကို သေချာ ပိတ်ပေးထားပါသည်) ---
LANG_DATA = {
    "မြန်မာ": {
    ...
    "English": {
        ...
        "storm_note": "📝 Note: Beware of lightning if thunderstorm > 60%.",
...
def load_stations():
    try:
        ...
        # Lat/Lon နေရာမှားနေပါက ဤနေရာတွင် float(row['Lat']) နှင့် float(row['Lon']) ကို လဲလှယ်ပါ
...
if df_h is not None:
    ...
    if mode_index == 1:
        ...
        if idx_choice == "အမြင့်ဆုံးအပူချိန်":
            val, th = tmax_val, [42, 40, 38]
        elif idx_choice == "Heat Index":
            val, th = t_now['HI'], [41, 38, 35]
        elif idx_choice == "WBGT":
            val, th = t_now['WBGT'], [32, 30, 28]
        else:
            val, th = t_now['UTCI'], [38, 32, 26]
        ...
        st.markdown(f"<div style='background-color:{bg}; padding:20px; border-radius:10px; text-align:center;'><h1>{T['risk_levels'][lvl]}</h1><h3>{idx_choice}: {val:.1f} °C</h3></div>", unsafe_allow_html=True)
    ...
    for i, city in enumerate(city_list):
        ...
        if dh is not None:
            for d in dd['Date']:
                ...
                all_data.append({'Date': d.strftime('%Y-%m-%d'), 'Station': city, 'Max_Temp': dd.loc[dd['Date'] == d, 'Tmax'].values[0] + bias, 'Max_HI': day_indices['HI'].max()})
        time.sleep(1.5) # API အားနားပေးရန်
...
st.markdown("<div style='text-align: center; color: gray;'>Official System: DMH Myanmar</div>", unsafe_allow_html=True)
